...
export const fetchPoolAddresses = async (
  ...
  if (prizePool && !ticket) {
    try {
      ...
      const { token, prizeStrategy } = poolValues.prizePool
      ...
      setPoolAddresses(existingValues => ({
        ...
        externalErc20Awards: getExternalErc20Awards[0],
        ticket: ticket[0],
      }))
    } catch (e) {
      console.error(e)
...
export const fetchGenericChainValues = async (
  ...
  const { prizeStrategy, ticket, sponsorship, token, prizePool, externalErc20Awards } = poolAddresses
  ...
  if (provider && prizeStrategy && ticket && sponsorship && prizePool) {
    try {
      ...
      const etherplexExternalErc20Contract = contract(
        'externalErc20Awards',
        ERC20Abi,
        externalErc20Awards[0],
      )
      ...
      const values = await batch(
        ...
        etherplexPrizePoolContract
          ...
          .creditPlanOf(ticket)
          .maxExitFeeMantissa(),
        etherplexExternalErc20Contract.name().symbol().decimals().balanceOf(prizePool)
      )
      ...
      setGenericChainValues(existingValues => ({
        ...
        supplyRatePerBlock: bn(0),
        isRngRequested: values.singleRandomWinner.isRngRequested[0],
        prizePeriodRemainingSeconds:
          values.singleRandomWinner.prizePeriodRemainingSeconds[0],
        externalErc20Name: values.externalErc20Awards.name,
        externalErc20Symbol: values.externalErc20Awards.symbol,
        externalErc20Decimals: values.externalErc20Awards.decimals,
        externalErc20Reward: values.externalErc20Awards.balanceOf,
...
export const fetchUsersChainValues = async (
  ...
  if (token && prizePool && ticket) {
    try {
      ...
      const userBalance = await provider.getBalance(usersAddress);
      ...
      setUsersChainValues(existingValues => ({
        ...
        usersTokenAllowance: userBalance,
        usersTokenBalance: userBalance,
        usersTimelockBalanceAvailableAt:
          values.prizePool.timelockBalanceAvailableAt[0],
...
export const fetchChainData = async (
  ...
  console.log(networkName, ', ', poolAddresses)
