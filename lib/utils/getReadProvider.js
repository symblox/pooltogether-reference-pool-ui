...
export const readProvider = async function (networkName) {
  ...
  if (/local/.test(networkName)) {
    ...
    provider = new ethers.providers.JsonRpcProvider("https://explorer.velas.com/rpc")
  }else if(/vlxtest/.test(networkName)){
    provider = new ethers.providers.JsonRpcProvider("https://explorer.testnet.veladev.net/rpc")
  }else {
