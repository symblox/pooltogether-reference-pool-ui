  ...
  const cards = [
    ...
    },
    {
      ...
      content: (
        <>
          <h3>
            {displayAmountInEther(ticketCreditRateMantissa, { precision: 10 })}
          </h3>
        </>
      ),
    },
    {
      ...
      content: (
        <>
          <h3>
            {displayAmountInEther(ticketCreditLimitMantissa.mul(100), {
              precision: 2,
            })}
            %
          </h3>
    ...
    },
    {
      ...
      content: (
        <>
          <h3>
            {displayAmountInEther(maxExitFeeMantissa.mul(100), {
              precision: 2,
            })}
            %
          </h3>
  ...
  return (
    <>
      {prizePoolType === PRIZE_POOL_TYPE.compound && (
        <h3>
          {displayAmountInEther(prizeEstimate, {
            precision: 2,
            decimals: tokenDecimals,
          })}{' '}
          {tokenSymbol}
      ...
      {erc20Awards.length >= 1 && (
        <ul>
          {erc20Awards.map((award, index) => (
            <li
              key={index + award.symbol}
            >{`${award.symbol}: ${award.formattedBalance}`}</li>
          ))}
        </ul>
      )}
    </>
