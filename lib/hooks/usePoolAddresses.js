...
import SyxSingleWinnerAbi from '@symblox/pvlx-contracts/abis/SyxSingleWinner'
      ...
      if (prizePool && !ticket) {
        try {
          ...
          if (
            ...
            const poolValues = await batch(
              provider,
              etherplexPrizePoolContract.token().prizeStrategy().sponsor(),
            )
            const { token, prizeStrategy, sponsor } = poolValues.prizePool
            // Query Prize Strategy
            const etherplexPrizeStrategyContract = contract(
              'prizeStrategy',
              SyxSingleWinnerAbi,
